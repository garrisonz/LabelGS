...
for i in range(11):
    if i <=0:
        continue
    mask_sample_numbers.append(i * 2)

print(mask_sample_numbers)

scene_name = "bed"
for idx, mask_sample_number in enumerate(mask_sample_numbers):

    version = start_version + idx

    cmd = (f"python train.py -s dataset/{dataset_name}/{scene_name} -m output/{dataset_name}/auto_{scene_name}_segEval{version} --eval --iteration {iteration} --label {start_cmd} --occlude_flag --mask_version {mask_version} --gpf_flag --mask_sample_number {mask_sample_number}") 
    print(cmd)
    os.system(cmd)
    #exit()
