...
if __name__ == '__main__':
    ...
    # parent folder of args.img_path
    img_path = args.img_path
    img_path = os.path.dirname(img_path)
    mapping = img_path + "/rendering_gt_mapping.json"
    # if mapping exists, copy to output
    if os.path.exists(mapping):
        os.system(f"cp {mapping} {out_path}")
        print(f"cp {mapping} {out_path}")
